...
CREATE DOMAIN ISBN_TYPE10 AS VARCHAR(10) CHECK (VALUE ~ '^\d{10}$');
CREATE DOMAIN ISBN_TYPE13 AS VARCHAR(13) CHECK (VALUE ~ '^\d{13}$');
CREATE DOMAIN RATING_TYPE AS DECIMAL(2, 1) CHECK (
    VALUE >= 0.0
    AND VALUE <= 5.0
);
CREATE DOMAIN URL_TYPE AS VARCHAR(2048) CHECK (VALUE ~ '^https?://');
-- Publisher Table
CREATE TABLE Publisher (
    publisher_id SERIAL PRIMARY KEY,
    name VARCHAR(200) NOT NULL UNIQUE,
    description TEXT,
    founding_year INTEGER CHECK (
        founding_year BETWEEN 1400 AND EXTRACT(
            YEAR
            FROM CURRENT_DATE
        )
    ),
...
CREATE TABLE Author (
    ...
    CONSTRAINT author_dates_check CHECK (
        death_date IS NULL
...
CREATE TABLE Book (
    ...
    publication_year INTEGER CHECK (
        ...
        AND publication_year <= EXTRACT(
            YEAR
            FROM CURRENT_DATE
        )
    ),
    publisher_id INTEGER REFERENCES Publisher(publisher_id) ON DELETE
    SET NULL ON UPDATE CASCADE,
        ...
        --    ratings_breakdown JSONB, -- Optional, unavailable for now
        series_id INTEGER REFERENCES Series(series_id) ON DELETE
    SET NULL
);
-- Physical Book Table
CREATE TABLE PhysicalBook (
    ...
    format format_type NOT NULL --    printing_house VARCHAR(200), -- Optional, unavailable for now
...
CREATE TABLE EBook (
    ...
    drm_protected BOOLEAN DEFAULT true --    supported_devices TEXT[], -- Optional, unavailable for now
    --    download_format VARCHAR(50)[] -- Optional, unavailable for now
);
-- AudioBook Table
CREATE TABLE AudioBook (
    book_id INTEGER PRIMARY KEY REFERENCES Book(book_id) ON DELETE CASCADE ON UPDATE CASCADE,
    duration_seconds INTEGER CHECK (duration_seconds > 0) --    narrator VARCHAR(200), -- Optional, unavailable for now
...
CREATE TABLE Genre (
    ...
    description TEXT,
    parent_genre_id INTEGER REFERENCES Genre(genre_id) ON DELETE
    SET NULL
);
...
CREATE TABLE PriceHistory (
    ...
    end_date TIMESTAMP,
    -- Optional, uncommented for use in views
    source VARCHAR(50),
    CONSTRAINT price_dates_check CHECK (
        end_date IS NULL
...
CREATE TABLE Reviews (
    ...
    text TEXT,
    spoiler BOOLEAN DEFAULT FALSE --    review_date DATE -- Optional, unavailable for now
);
...
CREATE TABLE DigitalContent (
    ...
    content_url URL_TYPE --    format VARCHAR(50), -- Optional, unavailable for now
